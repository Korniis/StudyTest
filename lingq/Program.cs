namespace lingq
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Employee employee2 = new Employee() { Id = 1, Name = "123", Gender = true, salary = 34, Age = 131 };
         ...
         /*   Console.WriteLine(e1);*/
            ...
            var name = list.Select(e => e.Name + "," + e.Age);
            foreach (var item in name) { Console.WriteLine(item); }
            var names = list.Select(e => new { kkk = e.Name });
            foreach (var item in names)
            {
                Console.WriteLine(item.kkk);

            }
            var item32 = list.GroupBy(e => e.Age).Select(g => new { g.Key, Maxs = g.Max(e => e.salary), mins = g.Max(e => e.salary), re = g.Count() });

            foreach (var item in item32)
            {
                Console.WriteLine(item.Maxs);
            }

            var list2 = list.Where(e => e.Id > 2).GroupBy(e => e.Age)
                .OrderBy(g => g.Key).Take(3).
                Select(g => new { dddd = g.Key, RS = g.Count(), PJ = g.Average(e => e.salary) });
